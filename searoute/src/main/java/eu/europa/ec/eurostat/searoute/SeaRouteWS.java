...
public class SeaRouteWS extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			case "rou":
				...
				boolean allowNonIRTC = "1".equals( request.getParameter("nonIRTC"));

				System.out.println("allowNonIRTC is "+allowNonIRTC);
				...
				if(oLon.length==0)
					...
					returnRoute(out, oLon[0], oLat[0], dLon[0], dLat[0], distP, geomP, sr, allowSuez, allowPanama, allowNonIRTC);
					for(int i=1; i<oLon.length; i++){
						out.print(",");
						returnRoute(out, oLon[i], oLat[i], dLon[i], dLat[i], distP, geomP, sr, allowSuez, allowPanama, allowNonIRTC);
					}
					if(oLon.length>1) out.print("]");
				}
				break;
	...
	private void returnRoute(PrintWriter out, double oLon, double oLat, double dLon, double dLat, boolean distP, boolean geomP, SeaRouting sr, boolean allowSuez, boolean allowPanama, boolean allowNonIRTC) {
		try {
			...
			Feature f = sr.getRoute(oPos, oN, dPos, dN, allowSuez, allowPanama, allowNonIRTC);
